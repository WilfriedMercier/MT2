!Mercier Wilfried
!07/01/2019
!Equation de burger - V2
!Derivee a differences finies ajoutees + boucle temporelle
...
PROGRAM Burger
   ...
   !...Declaration...
      ...
      REAL(KIND=8), PARAMETER           :: RE      = 1000.0_8        !Nb de Reynolds
      REAL(KIND=8), PARAMETER           :: x0      = 0.0_8           !Premier bord
      REAL(KIND=8), PARAMETER           :: x1      = 1.0_8           !Second bord
      REAL(KIND=8), PARAMETER           :: TotTime = 2.0_8           !Temps total
   ...
   CALL Write2TabPlusToFile(tab_x, tab_v, 0.0_8, NSTEPX, 11)
   DO t=1, NSTEPT
      DO x=2, NSTEPX+1
         !Calcul premiere et seconde derivees
!         WRITE(*,*)t, x, tab_v(x-1), fder, sder

         !Verifie qu'il n'y a pas de NaN
         IF ( ISNAN(tab_v(x+1)) ) THEN
            RETURN
         ENDIF
      ...
      IF ( MOD(t, 500) .eq. 0 ) THEN
         CALL Write2TabPlusToFile(tab_x, tab_v, REAL(t*stept, KIND=8), NSTEPX, 11)
      ENDIF
   END DO
   CLOSE(11)
...
SUBROUTINE Write2TabPlusToFile(tab1, tab2, time, length, unitOut)
   ...
   !...Declaration...
   !...non locales...
   INTEGER,                    INTENT(IN) :: length, unitOut         !Taille tableau et unite sortie
   REAL(KIND=8),               INTENT(IN) :: time                    !Temps
   ...
   !Ecriture fichier
   DO i=1, length
      WRITE(unitOut, '(F13.6, 4X, F13.2, 4X, F13.2)')time, tab1(i), tab2(i)
   END DO
