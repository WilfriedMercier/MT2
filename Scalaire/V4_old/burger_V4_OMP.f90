...
PROGRAM Burger
   ...
   !...Declaration...
      ...
      INTEGER,      PARAMETER           :: NSTEPX = 600           !Nombre de pas selon x
      !...Tableaux...
   ...
   !$OMP PARALLEL PRIVATE(fder, sder, temp, tempMin)
   DO t=1, NSTEPT
   ...
   !$OMP DO SCHEDULE(STATIC)
      DO x=2, NSTEPX+1
         ...
         !Met a jour la case du bon tableau
         IF ( MOD(t, 2) .ne. 0 ) THEN
            tab_v(x) = stept*(sder/RE - tab_v(x)*fder) + tab_v(x)
         ELSE
            tab_temp(x) = stept*(sder/RE - tab_temp(x)*fder) + tab_temp(x)
         ENDIF
      END DO
   !$OMP END DO
      
      !Ecrit dans fichier
      IF ( MOD(t, 10*NSTEPX) .eq. 0 ) THEN
...
SUBROUTINE Write2TabPlusToFile(tab1, tab2, time, length, unitOut)
   ...
   !Ecriture fichier
   DO i=1, length
      WRITE(unitOut, '(I15, 4X, F13.2, 4X, F13.2)')time, tab1(i), tab2(i)
   END DO
