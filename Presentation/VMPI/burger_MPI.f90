...
PROGRAM Burger
   ...
   CALL Write2TabPlusToFile(tab_x, tab_v, 0, NSSSTEPX, unitOut)
   ...
   DO t=1, NSTEPT
      ...
      !Transfert tampons des deux tableaux
      IF (isOdd) THEN

         !Envoie du membre le plus a droite
         IF (proc .ne. maxproc) THEN
            CALL MPI_ISEND(tab_v(NSSSTEPX), 1, MPI_DOUBLE_PRECISION, proc+1, proc, MPI_COMM_WORLD, requete, err)
            CALL MPI_WAIT(requete, statut, err)
         ENDIF
         
         !Reception du membre le plus a gauche
         IF (proc .ne. 0) THEN
            CALL MPI_IRECV(tab_v(1), 1, MPI_DOUBLE_PRECISION, proc, proc-1, MPI_COMM_WORLD, statut, requete, err)
            CALL MPI_WAIT(requete, statut, err)
         ENDIF       
         
         !Envoie du membre le plus a gauche
         IF (proc .ne. 0) THEN
            CALL MPI_ISEND(tab_v(1), 1, MPI_DOUBLE_PRECISION, proc-1, proc, MPI_COMM_WORLD, requete, err)
            CALL MPI_WAIT(requete, statut, err)                        
         ENDIF
         
         !Reception du membre le plus a droite
         IF (proc .ne. maxproc) THEN
            CALL MPI_IRECV(tab_v(NSSSTEPX), 1, MPI_DOUBLE_PRECISION, proc, proc+1, MPI_COMM_WORLD, statut, requete, err)
            CALL MPI_WAIT(requete, statut, err)
         ENDIF        
      ...
      ELSE
         ...
         !Envoie du membre le plus a droite
         IF (proc .ne. maxproc) THEN
            CALL MPI_ISEND(tab_temp(NSSSTEPX), 1, MPI_DOUBLE_PRECISION, proc+1, proc, &
			   MPI_COMM_WORLD, requete, err)
            CALL MPI_WAIT(requete, statut, err)
         ENDIF

         !Reception du membre le plus a gauche
         IF (proc .ne. 0) THEN
            CALL MPI_IRECV(tab_temp(1), 1, MPI_DOUBLE_PRECISION, proc, proc-1, MPI_COMM_WORLD, &
			   statut, requete, err)
         ...
         !Envoie du membre le plus a gauche
         IF (proc .ne. 0) THEN
            CALL MPI_ISEND(tab_temp(1), 1, MPI_DOUBLE_PRECISION, proc-1, proc, MPI_COMM_WORLD, requete, err)
            CALL MPI_WAIT(requete, statut, err)
         ENDIF

         !Reception du membre le plus a droite
         IF (proc .ne. maxproc) THEN
            CALL MPI_IRECV(tab_temp(NSSSTEPX), 1, MPI_DOUBLE_PRECISION, proc, proc+1, &
		           MPI_COMM_WORLD, statut, err)
            CALL MPI_WAIT(requete, statut, err)
         ENDIF
      ENDIF         
      ...
      IF ( MOD(t, INT(NSTEPT/10)) .eq. 0 ) THEN
         IF ( MOD(t, 2) .ne. 0 ) THEN
            CALL Write2TabPlusToFile(tab_x, tab_v, t, NSSSTEPX, unitOut)
         ELSE
            CALL Write2TabPlusToFile(tab_x, tab_temp, t, NSSSTEPX, unitOut)
         ENDIF
      ENDIF
   END DO
   CLOSE(unitOut)
